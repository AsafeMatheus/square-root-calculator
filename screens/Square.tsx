...
export default function({navigation}:any){
    ...
    function showAnswer(number:any){
        if (number.length > 0){
            return(
                <Text style={styles.answerText}>{Math.sqrt(Number(number)).toFixed(2)}</Text>
            )
        }
    }
    ...
    function showMethod(number: any){
        if(number.length > 0){
            ...
            let squareRootOfNumber = Math.sqrt(Number(number))
        
            if(Number.isInteger(squareRootOfNumber) == false){
                return(
                    <Text style={styles.noMethodText}>A raiz quadrada{'\n'} de {number} não é{'\n'} um número inteiro</Text>
                )
            } else{
